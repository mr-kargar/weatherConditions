            ...
            <div className={`border-2 border-sky-400 rounded-t-2xl ${show ? 'block' : 'hidden'}`}  >
                <div className="flex flex-col item-center ">
                    <div className=" flex justify-between bg-sky-400 rounded-t-xl py-2 px-6">
                        <div >
                            <strong>City :</strong>
                            <strong className='ml-2 text-pink-600'>{props.props.resolvedAddress}</strong>
                        </div>
                        <div className='flex items-center gap-4'>
                            <strong className='hidden md:flex '>Update Time : {props.props.currentConditions.datetime}</strong>
                    ...
                    <div className='flex flex-col sm:flex-row items-center justify-center p-4'>
                        <img src={process.env.PUBLIC_URL + "/icons/" + props.props.currentConditions.icon + ".svg"} alt={props.props.currentConditions.icon} className=' h-32 w-32' />
                        <div className='flex flex-col lg:flex-row md:gap-8 ml-10'>
                            <div className="content">
                                <span className='font-bold mr-2'>Weather condition : </span>
                                <span className='font-medium text-pink-600'>{props.props.currentConditions.conditions}</span>
                            </div>
                            <div className="content">
                                <span className='font-bold mr-2'>Temperature :</span>
                                <span className='font-medium text-pink-600'>{props.props.currentConditions.temp}</span>
                            </div>
                            <div className="content">
                                <span className='font-bold mr-2'>Humidity : </span>
                                <span className='font-medium text-pink-600'>{props.props.currentConditions.humidity} %</span>
                            </div>
                        </div>
                    </div>
                </div>
